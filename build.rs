...
fn main() {
    ...
    for s in shaders.iter() {
        ...
        if let Ok(binary) = compile_result {
            ...
            println!(
                "cargo:warning=Shader '{}' failed to compile due to:",
                s.display()
            );
            match compile_result.err().unwrap() {
                shaderc::Error::CompilationError(_, msg) => {
                    println!("cargo:warning=Compile Error:--\n{}--", msg)
                }
                shaderc::Error::InternalError(msg) => {
                    println!("cargo:warning=Internal Error: {}", msg)
                }
                shaderc::Error::InvalidStage(msg) => {
                    println!("cargo:warning=Invalid Stage: {}", msg)
                }
                shaderc::Error::InvalidAssembly(msg) => {
                    println!("cargo:warning=Invalid Assembly: {}", msg)
                }
                shaderc::Error::NullResultObject(msg) => {
                    println!("cargo:warning=Null Result Object: {}", msg)
                }
            }
